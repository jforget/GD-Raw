...
sub gdImageRed($im, Int $c) is export {
    $im.trueColor ?? gdTrueColorGetRed($c) !! "\$im.red$c".EVAL
}
sub gdImageGreen($im, Int $c) is export {
    $im.trueColor ?? gdTrueColorGetGreen($c) !! "\$im.green$c".EVAL
}
sub gdImageBlue($im, Int $c) is export {
    $im.trueColor ?? gdTrueColorGetBlue($c) !! "\$im.blue$c".EVAL
}
sub gdImageAlpha($im, Int $c) is export {
    $im.trueColor ?? gdTrueColorGetAlpha($c) !! "\$im.alpha$c".EVAL
}
...
sub gdImageBmpPtr(gdImageStruct $image, int32 $size is rw, int32 $compression)
    returns OpaquePointer
    is native(LIB) is export { ... }

sub gdImageGdPtr(gdImageStruct $image, int32 $size is rw)
    returns OpaquePointer
    is native(LIB) is export { ... }

sub gdImageGifPtr(gdImageStruct $image, int32 $size is rw)
    returns OpaquePointer
    is native(LIB) is export { ... }

sub gdImageJpegPtr(gdImageStruct $image, int32 $size is rw, int32 $quality)
    returns OpaquePointer
    is native(LIB) is export { ... }
...
sub gdImagePngPtrEx(gdImageStruct $image, int32 $size is rw, int32 $compression)
    returns OpaquePointer
    is native(LIB) is export { ... }

sub gdImageTiffPtr(gdImageStruct $image, int32 $size is rw)
    returns OpaquePointer
    is native(LIB) is export { ... }

sub gdImageWebpPtr(gdImageStruct $image, int32 $size is rw)
    returns OpaquePointer
    is native(LIB) is export { ... }

sub gdImageWepbPtrEx(gdImageStruct $image, int32 $size is rw, int32 $quality)
    returns OpaquePointer
    is native(LIB) is export { ... }
...
provide you with an rakuish interface, but tries to stay as close to its C<C>
origin as possible.
...
When  using a  function  C<gdImage>I<XXX>C<Ptr> to  fill  a blob  with
graphic data,  the memory  management function is  C<gdFree>. Usually,
the  pointer  has a  very  short  lifespan.  Once  the blob  has  been
generated by C<blob-from-pointer>,  the pointer is useless  and can be
fred immediately. A typical chunk of code would be:
